...
(defn fix-path
  "Coverts the passed path name to a name that uses the path-separator
  which is appropriate for the platform. On MS Windows
  \"src/namespace\" becomes \"src\\namespace\", on Linux it remains
  unchanged."
  [path]
  (.getPath (File. path)))
...
(defn generate
  ...
  ([cljx-path output-path extension rules]
     (let [cljx-path (fix-path cljx-path)
           output-path (fix-path output-path)]
